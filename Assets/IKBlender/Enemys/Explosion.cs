...
public class Explosion : MonoBehaviour
{
	...
	[SerializeField] float _futtobiPower;

	[Header("爆風の判定が実際に発生するまでのディレイ")]
	[SerializeField] float _startDelaySeconds = 0.1f;

	[Header("爆風の持続フレーム数")]
	[SerializeField]  int _durationFrameCount = 1;

	[Header("エフェクト含めすべての再生が終了するまでの時間")]
	[SerializeField] float _stopSeconds = 2f;
	...
	private void OnTriggerEnter(Collider other)
	{
		...
		CharacterController.Move(direction * _futtobiPower);
	}
}
